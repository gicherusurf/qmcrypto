...
import { LogOut, User, Shield, Menu, X, Radio, History, Trophy, Store } from "lucide-react";
import { useState } from "react";

export function Navbar() {
  const { user, isAdmin, isModerator, isFranchise, signOut } = useAuth();
          ...
          <div className="hidden md:flex items-center gap-2">
            {user ? (
              <>
                ...
                {isFranchise && !isAdmin && <Link to="/franchise"><Button variant="ghost" size="sm"><Store className="h-4 w-4 mr-2" />My Franchise</Button></Link>}
            ...
            <div className="flex flex-col gap-2">
              {user ? (
                <>
                  ...
                  {isFranchise && !isAdmin && <Link to="/franchise" onClick={() => setOpen(false)}><Button variant="ghost" className="w-full justify-start"><Store className="h-4 w-4 mr-2" />My Franchise</Button></Link>}
